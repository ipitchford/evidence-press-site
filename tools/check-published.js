...
const OBSERVATORY_META = path.join(ROOT, 'pages', 'observatory.json');
...
function unresolvedPublicationMetadata() {
  if (!fs.existsSync(OBSERVATORY_META)) return [];
  const source = fs.readFileSync(OBSERVATORY_META, 'utf8');
  return [...new Set(source.match(/__OBSERVATORY_[A-Z0-9_]+__/g) || [])].sort();
}
...
(async () => {
  ...
  console.log(green('Published-URL preservation check passed.'));

  const unresolved = unresolvedPublicationMetadata();
  if (unresolved.length) {
    console.error(red('\nREFUSING: Observatory publication metadata is not final.'));
    for (const token of unresolved) console.error(red(`  unresolved: ${token}`));
    console.error('\nReplace the exact values in pages/observatory.json, rebuild, and rerun this gate.');
    process.exit(1);
  }
