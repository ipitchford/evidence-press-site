...
echo "1/7  main site build  (build.js -> dist/)"
node build.js

echo "2/7  thumbnail gate    (every release -> repo copy; central mirror on maintainer Mac)"
node tools/make-thumb.js --check

echo "3/7  protocol gates    (schemas, examples, pilot controls, hostile fixtures)"
...
echo "4/7  protocols build  (protocols/build-protocols.js -> protocols/dist/)"
node protocols/build-protocols.js

echo "5/7  starter replay   (fresh extraction -> 31 pilot controls)"
node protocols/tools/test-starter-kit.js

echo "6/7  mount            (protocols/dist -> dist/protocols/)"
rm -rf dist/protocols
cp -R protocols/dist dist/protocols

echo "7/7  sitemap merge    (fold /protocols/ page URLs into dist/sitemap.xml)"
