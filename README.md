...
5. Edit `body.md` — the press release. Template sections: `## Summary` (popular), `## Summary for specialists`, `## Technical summary`, `## Who should care, and why` (audience table), `## The most valuable next projects`, `## Specialist audience candidates`, `## What is in the evidence package`. Use audience-respectful plain language; do not add headings framed as “Explain it like I’m five”, ELI5, or a five-year-old account. The release-page policy test rejects these labels across the complete corpus.
